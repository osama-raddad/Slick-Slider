...
class SimpleSlider(context: Context) : ObservableHorizontalScrollView(context) {
...
//                else -> animatedScroll(end, fastSpeed)
    ...
    private fun calculateTheStartAndEndOfLayout(callback: () -> Unit) {
        linearLayout?.post {
            ...
            start = calculateTheDistanceToTheMedalOfTheScreen()
            end = (calculateTheDistanceToTheMedalOfTheScreen() + blueAreaWidth.toInt())
...
//        scroll to the end
    ...
    private fun animatedScroll(position: Int, speed: Float, cb: () -> Unit = {}) {
        post {
            if (scrollAnimator != null) scrollAnimator?.pause()
