...
class RTLPartSlider(context: Context) : ObservableHorizontalScrollView(context) {
    ...
    private fun <T, K> addItemsToLayout(data: MutableMap<T, K>, onFinishInflating: () -> Unit = {}) {
        ...
        val size = if ((displacement > 0) &&
            ...
            data.size + partSize - 1 else data.size
    ...
    private fun <T, K> createViews(i: Int, data: MutableMap<T, K>) {
        ...
        item.post { itemWidth = item.width.toFloat() }
    ...
    private fun calculateTheStartAndEndOfLayout(callback: () -> Unit) {
        linearLayout?.post {
            ...
            end = (calculateTheDistanceToTheMedalOfTheScreen() + getDisplacementValue(complement = true))
            start = ((calculateTheDistanceToTheMedalOfTheScreen() + blueAreaWidth) - getDisplacementValue()).toInt()
